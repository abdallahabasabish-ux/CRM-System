// =============================================================
// reports.js - النسخة المستقرة مع تصحيح الأخطاء
// =============================================================
...
// =============================================================
function updateUI() {
  console.log('🔄 تحديث واجهة التقارير...');
...
function updateTreasuryMiniStats(treasuryData) {
  ...
  const elBalance = document.getElementById('treasuryHeaderBalance');
  const elDeposits = document.getElementById('miniTotalDeposits');
  const elWithdrawals = document.getElementById('miniTotalWithdrawals');
  const elTransfers = document.getElementById('miniTotalTransfers');
  const elCount = document.getElementById('miniTxCount');

  if (elBalance) elBalance.innerHTML = `<small>الرصيد الحالي</small> ${formatCurrency(balance)}`;
  if (elDeposits) elDeposits.textContent = formatCurrency(deposits);
  if (elWithdrawals) elWithdrawals.textContent = formatCurrency(withdrawals);
  if (elTransfers) elTransfers.textContent = formatCurrency(transfers);
  if (elCount) elCount.textContent = treasuryData.length;
}

// =============================================================
// 8.  الرسوم البيانية (مبسطة)
// =============================================================
...
function createCharts(orders, payments) {
  ...
  if (ctx1) {
    state.charts.status = new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [{
          data: Object.values(statusMap).length ? Object.values(statusMap) : [1],
          backgroundColor: colors.slice(0, Object.keys(statusMap).length || 1),
          borderWidth: 0
        }]
      },
  ...
  if (ctx2) {
    state.charts.revenue = new Chart(ctx2, {
      type: 'bar',
      data: {
        labels: Object.keys(monthlyRevenue).map(l => l.split('-')[1] + '/' + l.split('-')[0]),
        datasets: [{ label: 'الإيرادات', data: Object.values(monthlyRevenue), backgroundColor: 'rgba(255,102,0,0.7)', borderRadius: 6 }]
      },
  ...
  if (ctx3) {
    state.charts.services = new Chart(ctx3, {
      ...
      data: {
        ...
        datasets: [{ data: sorted.map(s => s[1]), backgroundColor: ['#ff6600','#0d6efd','#28a745','#8b5cf6','#ffc107','#dc3545'], borderWidth: 0 }]
      },
  ...
  const empData = state.employees.map(e => ({ name: e.name || 'غير معروف', count: empOrders[e.id] || 0 })).sort((a,b) => b.count - a.count).slice(0,8);
  ...
  if (ctx4) {
    state.charts.employees = new Chart(ctx4, {
      ...
      data: {
        ...
        datasets: [{ label: 'الطلبات', data: empData.map(e => e.count), backgroundColor: 'rgba(13,110,253,0.7)', borderRadius: 6 }]
      },
...
function updateOrdersReport(orders) {
  ...
  if (data.length === 0) { tbody.innerHTML = `<tr><td colspan="7" class="text-center text-muted">لا توجد طلبات</td></tr>`; return; }
  ...
  data.forEach(o => {
    ...
    const total = o.total || 0, paid = o.paid || 0, remaining = total - paid;
    totalSum += total; paidSum += paid; remainingSum += remaining;
    html += `<tr><td><strong>#${o.orderNumber || 'N/A'}</strong></td><td>${customer ? escapeHtml(customer.name) : 'غير معروف'}</td><td>${service ? escapeHtml(service.name) : 'غير معروف'}</td><td><span class="badge-status badge bg-${o.status === 'مكتمل' ? 'success' : o.status === 'ملغي' ? 'danger' : 'warning'}">${escapeHtml(o.status || 'جديد')}</span></td><td>${formatCurrency(total)}</td><td>${formatCurrency(paid)}</td><td>${formatCurrency(remaining)}</td></tr>`;
  });
...
function updateCustomersReport() {
  ...
  if (!tbody) return;
  if (state.customers.length === 0) { tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted">لا يوجد عملاء</td></tr>`; return; }
  ...
  state.customers.forEach(c => {
    ...
    totalPaidSum += totalPaid; totalBalanceSum += balance;
    html += `<tr><td>${escapeHtml(c.name || '')}</td><td>${escapeHtml(c.phone || '')}</td><td>${ordersCount}</td><td>${formatCurrency(totalPaid)}</td><td>${formatCurrency(balance)}</td></tr>`;
  });
...
function updateEmployeesReport() {
  ...
  if (!tbody) return;
  if (state.employees.length === 0) { tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted">لا يوجد موظفين</td></tr>`; return; }
  ...
  state.employees.forEach(e => {
    ...
    html += `<tr><td>${escapeHtml(e.name || '')}</td><td>${escapeHtml(e.position || '')}</td><td>${empOrders.length}</td><td>${rate}%</td><td>${formatCurrency(commission)}</td></tr>`;
  });
...
function updatePaymentsReport(payments) {
  ...
  if (data.length === 0) { tbody.innerHTML = `<tr><td colspan="4" class="text-center text-muted">لا توجد مدفوعات</td></tr>`; return; }
  ...
  data.forEach(p => {
    ...
    html += `<tr><td>${escapeHtml(customerName)}</td><td>${formatCurrency(p.amount || 0)}</td><td>${escapeHtml(p.method || '')}</td><td>${formatDate(p.paymentDate)}</td></tr>`;
  });
...
function updateTreasuryReport(treasuryData) {
  ...
  if (data.length === 0) { tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted">لا توجد معاملات خزينة</td></tr>`; return; }
  let html = '';
  ...
  data.forEach(tx => {
    ...
    html += `<tr class="${rowClass}"><td>${formatDate(tx.createdAt)}</td><td><span class="${badgeClass}">${typeLabel}</span></td><td class="tx-amount">${sign}${formatCurrency(amount)}</td><td>${escapeHtml(tx.source || tx.target || '-')}</td><td>${escapeHtml(tx.note || '')}</td></tr>`;
  });
  const balance = totalDeposits - totalWithdrawals + totalTransfers;
  html += `<tr class="report-total-row" style="background:var(--color-primary);color:#fff;"><td colspan="2" style="text-align:center;">الإجمالي</td><td>الإيداعات: ${formatCurrency(totalDeposits)}</td><td>السحوبات: ${formatCurrency(totalWithdrawals)}</td><td>الرصيد: ${formatCurrency(balance)}</td></tr>`;
  tbody.innerHTML = html;
}

// =============================================================
// 10.  التصدير (Excel, PDF, طباعة)
// =============================================================
...
function exportToExcel() {
  const container = getActiveTable();
  if (!container) { showToast('لا يوجد جدول لتصديره', 'warning'); return; }
  ...
  } catch (e) { showToast('خطأ في تصدير Excel', 'error'); }
}
...
async function exportToPdf() {
  ...
  } catch (e) { showToast('خطأ في تصدير PDF', 'error'); }
}

function printReport() {
  const container = getActiveTable();
  if (!container) { showToast('لا يوجد جدول للطباعة', 'warning'); return; }
  const win = window.open('', '_blank');
  win.document.write(`<html><head><title>تقرير</title><style>body{font-family:'Almarai',sans-serif;direction:rtl;padding:20px;}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #ddd;padding:8px;text-align:right;}th{background:#f5f5f5;}</style></head><body>${container.innerHTML}</body></html>`);
  win.document.close();
  win.print();
}
...
async function init() {
  ...
  onAuthStateChangedCallback(async (user) => {
    ...
    if (sidebarAvatar) sidebarAvatar.textContent = user.displayName ? user.displayName.charAt(0).toUpperCase() : user.email.charAt(0).toUpperCase();
